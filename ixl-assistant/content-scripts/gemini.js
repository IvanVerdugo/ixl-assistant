...
if (chrome && chrome.storage && chrome.storage.sync) {
	chrome.storage.sync.get({ ixlDebug: false }, (items) => {
		...
		if (ixlDebug) console.log('Gemini script: debug enabled');
	});
	chrome.storage.onChanged.addListener((changes) => {
		if (changes.ixlDebug) {
			ixlDebug = !!changes.ixlDebug.newValue;
			console.log('Gemini script: debug changed ->', ixlDebug);
		}
	});
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	if (message.type === "receiveQuestion") {
		...
		if (ixlDebug) console.log('Gemini script: received question', message.question);
	...
	observer = new MutationObserver((mutations) => {
		...
		try {
			...
			if (parsed.answer && !hasResponded) {
				hasResponded = true;
				if (ixlDebug) console.log('Gemini script: sending response to background', responseText);
				chrome.runtime
					.sendMessage({
						type: "geminiResponse",
					...
					.catch((error) => {
						if (ixlDebug) console.error("Gemini script: Error sending response:", error);
					});
			}
		} catch (e) {
			const isGenerating =
