...
if (chrome && chrome.storage && chrome.storage.sync) {
	chrome.storage.sync.get({ ixlDebug: false }, (items) => {
		...
		if (ixlDebug) console.log('ChatGPT script: debug enabled');
	});
	chrome.storage.onChanged.addListener((changes) => {
		if (changes.ixlDebug) {
			ixlDebug = !!changes.ixlDebug.newValue;
			console.log('ChatGPT script: debug changed ->', ixlDebug);
		}
	});
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	if (message.type === "receiveQuestion") {
		...
		if (ixlDebug) console.log('ChatGPT script: received question', message.question);
	...
	observer = new MutationObserver((mutations) => {
		...
		try {
			...
			if (parsed.answer && !hasResponded) {
				hasResponded = true;
				if (ixlDebug) console.log('ChatGPT script: sending response to background', responseText);
				chrome.runtime
					.sendMessage({
					...
					.catch((error) => {
						if (ixlDebug) console.error("ChatGPT script: Error sending response:", error);
					});
			}
		} catch (e) {
