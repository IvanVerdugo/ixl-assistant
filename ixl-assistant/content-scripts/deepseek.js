...
if (chrome && chrome.storage && chrome.storage.sync) {
	chrome.storage.sync.get({ ixlDebug: false }, (items) => {
		...
		if (ixlDebug) console.log('DeepSeek script: debug enabled');
	});
	chrome.storage.onChanged.addListener((changes) => {
		if (changes.ixlDebug) {
			ixlDebug = !!changes.ixlDebug.newValue;
			console.log('DeepSeek script: debug changed ->', ixlDebug);
		}
	});
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	if (message.type === "receiveQuestion") {
		...
		if (ixlDebug) console.log('DeepSeek script: received question', message.question);
...
function processResponse(responseText) {
	...
	try {
		...
		if (parsed && parsed.answer && !hasResponded) {
			hasResponded = true;
			if (ixlDebug) console.log('DeepSeek script: sending response', cleanedText);
			chrome.runtime
				.sendMessage({
				...
				.catch((error) => {
					if (ixlDebug) console.error('DeepSeek script: sendMessage error', error);
					return false;
				});
